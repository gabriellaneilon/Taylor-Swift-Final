...
ggplot(Clean, aes(x = album, y = popularity, fill = album)) +
  ...
  scale_fill_manual(
    values = c("lightblue", "gold", "purple", "red", "blue", "green", "pink", "grey", "darkorange3", "darkblue")
  ) +
...
Common characteristics of top 5 songs per album
```{r, echo=FALSE}
...
# popular_words %>%
#   ggplot(aes(row, n)) +
#     geom_col(show.legend = NULL, alpha=.75) +
...
#####
# Filter the data to include only the top 10 most popular songs
top_songs <- Clean %>%
  filter(!is.na(album)) %>%
  arrange(desc(popularity)) %>%
  slice_head(n = 20)

# Extract the desired characteristics for the top 10 songs
...
# Print the common characteristics
print(common_df)

ggplot(common_df, aes(x=Variable, y=Common_Values, fill=Common_Values))+
    geom_bar(stat="identity", show.legend = FALSE)+
    coord_flip()+
    labs(x="Variable", y="Common Values",
         title="Common Characteristics of Top 20 Songs")+
    theme_minimal()+
    theme(axis.text.x = element_blank())
```

Based on common characteristics look at 
```{r, echo=FALSE}
library(ggplot2)
library(dplyr)
...
# Example usage
  ...
  create_pie_chart(Clean, tempo, "Tempo")
  create_pie_chart(Clean, valence, "Valence")
...
###
# train_data_t <- train_data %>% dplyr::select(hit,name,acousticness, danceability, energy, instrumentalness, liveness, loudness, speechiness, tempo, valence,duration_ms)
# test_data_t <- test_data %>% dplyr::select(hit,name,acousticness, danceability, energy, instrumentalness, liveness, loudness, speechiness, tempo, valence,duration_ms)
# 
# # Train the Random Forest model
# model_t <- ranger(hit ~ ., data = train_data_t)
# 
# # Make predictions on the test set
# predictions_t <- predict(model_t, data = test_data_t)$predictions
# 
# # Add the predictions to the test_data
# test_data_t$song_prediction_t <- predictions_t
# 
# # Sort the test data by predicted probabilities in descending order
# sorted_data_t <- test_data_t[order(-test_data_t$song_prediction_t), ]
# 
# # Select the top 3 songs
# top_3_hits_t <- head(sorted_data_t, 3)
# 
# # Print the top 3 hits
# print(top_3_hits_t$name)
#######
...
# Print the top 3 hits
print(top_5_hits$name)

knitr::kable(
      top_5_hits$name, col.names = c('Song Name'),caption = "Next hits predicted")
...
# # Assuming you have already split your data into training and test datasets
# # X_train and X_test are the feature matrices, and Y_train and Y_test are the target vectors
# 
# # Get common column names between X_train and X_test
...
# 
# # Train the random forest model
# model <- ranger(Y_train ~ ., data = cbind(X_train, Y_train))
# 
# # Predict on the test set
# predictions <- predict(model, data = X_test)$predictions
# 
# # Combine the predicted values with the test data
# predicted_data <- cbind(X_test, predicted_song_name = predictions)
# 
# # Show the top 3 predicted songs
# top_predicted_songs <- head(predicted_data[order(-predictions), ], 3)
...
# Print the top 3 hits
print(top_5_hits_best$name)

knitr::kable(
      top_5_hits_best$name, col.names = c('Song Name'),caption = "Next hits predicted")

```



Look at the best predictors
...
p1 <- vip::vip(rf_impurity, num_features = 10, bar = FALSE, aesthetics = list(color = "darkred", fill = "darkred"),
    all_permutations = TRUE, jitter = TRUE) +ggtitle("Impurity-based variable importance")
p2 <- vip::vip(rf_permutation, num_features = 10, bar = FALSE, aesthetics = list(color = "darkred", fill = "darkred"))
...
Impurity-based importance and permutation-based importance are two different approaches for estimating the feature importance in a Random Forest model.

Impurity-Based Importance:
Impurity-based importance, also known as Gini importance or mean decrease impurity, is a commonly used method to measure feature importance in Random Forests. It assesses the importance of a feature by evaluating how much the impurity or randomness in the predictions is reduced by splitting on that feature. The impurity measure, such as Gini impurity or entropy, quantifies the disorder or lack of purity in a set of samples. The idea is that if a feature is important, it will create more effective splits that reduce the impurity and lead to more accurate predictions.
The impurity-based importance is computed during the construction of the Random Forest model. It calculates the total reduction in impurity over all decision trees in the forest for each feature. The importance values are then scaled so that the sum of all feature importances equals 100%.

Permutation-Based Importance:
Permutation-based importance, also known as mean decrease accuracy or mean decrease performance, is an alternative approach to estimate feature importance in Random Forests. It evaluates the importance of a feature by measuring the decrease in the model's performance when the values of that feature are randomly shuffled or permuted.
The permutation-based importance is calculated after the Random Forest model is trained. The procedure involves permuting the values of a single feature while keeping the other features unchanged and measuring the decrease in the model's accuracy or another performance metric. If a feature is important, permuting its values will disrupt the relationship with the target variable, resulting in a significant drop in the model's performance.

Compared to impurity-based importance, permutation-based importance provides a more direct measure of a feature's contribution to the model's predictive power. It considers the effect of a feature in the context of the entire model



n simpler terms, impurity-based importance tells us how much each feature contributes to making accurate predictions in a Random Forest model by measuring how well it helps to split the data into pure subsets.In simpler terms, permutation-based importance measures the importance of a feature by checking how much the model's performance suffers when the values of that feature are randomly mixed up. If shuffling a feature's values causes a big drop in performance, it means that feature is important for accurate predictions.




Permutation feature importance is a way to measure the importance of a feature in a model. It does this by checking how much the model's prediction error increases when we randomly shuffle or mix up the values of that feature, which breaks the connection between the feature and the true outcome we're trying to predict.
...
roc_plot <- ggplot(roc_data, aes(x = roc_data$X1...as.numeric.roc_obj.specificities., y = roc_data$as.numeric.roc_obj.sensitivities.)) +
  geom_path() +
...
# Create the prediction plot (scatter plot)
prediction_plot <- ggplot(test_data, aes(x = song_prediction_best, y = name_n)) +
  geom_point() +
...
Higher x-values indicate higher false positives than true negatives. higher y-axis indicate higher number of true positives than false negatives
...
The more that the ROC curve hugs the top left corner of the plot, the better the model does at classifying the data into categories.A model with an AUC equal to 0.5 would be a perfectly diagonal line and it would represent a model that is no better than a model that makes random classifications
Data: predictions_best in 12 controls (sorted_data_best$hit 0) < 8 cases (sorted_data_best$hit 1).
